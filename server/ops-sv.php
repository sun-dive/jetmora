...
//
// ── THE `SV` SET: opcode numbering ───────────────────────────────────────────────────────────────────
// BSV numbering. ⚠ This is NOT jetmora's set and NOT Bitcoin 0.1.3's: BSV placed SPLIT/NUM2BIN/BIN2NUM
// at 0x7f–0x81, overwriting 0.1.3's SUBSTR/LEFT/RIGHT. That difference is the whole reason the sets are
// separate files (gaps §10.9) — a fix here must not be able to reach `BT` or `JF`.
//
// ⚠⚠ ISOLATION IS THE POINT. Nothing in this file may be imported by another set's interpreter, and
// nothing from another set may be imported here. The only thing that crosses the boundary is the
...
const SV_OP = [
  ...
  'OP_CAT'=>0x7e,
  // ⚠⚠ HERE IS THE DIVERGENCE FROM 0.1.3 — and the reason `BT` is a different file entirely.
  'OP_SPLIT'=>0x7f,      // 0.1.3 has OP_SUBSTR at this number
  'OP_NUM2BIN'=>0x80,    // 0.1.3 has OP_LEFT
  'OP_BIN2NUM'=>0x81,    // 0.1.3 has OP_RIGHT
  'OP_SIZE'=>0x82,
...
/** number → name, for error messages and for `unbasic`-style rendering. */
function sv_op_name(int $n): string {
  static $rev = null;
  if ($rev === null) $rev = array_flip(SV_OP);
